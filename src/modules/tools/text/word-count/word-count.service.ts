import type { WordCountInput, WordCountOutput } from './word-count.schema.js'

export const calculateWordCount = function (input: WordCountInput): WordCountOutput {
  return {
    textLengthRaw: calculateTextLengthRaw(input),
    textLengthNoSpace: calculateTextLengthNoSpace(input),
    wordCount: calculateWords(input),
...
// Text length raw
const calculateTextLengthRaw = function (input: WordCountInput): number {
  return input.length
}

// Text length without spaces
const calculateTextLengthNoSpace = function (input: WordCountInput): number {
...
const calculateWords = function (input: WordCountInput): number {
...
const calculateSentenceCount = function (input: WordCountInput): number {
  if (isWhitespaceString(input)) return 0
...
const calculateNsCount = function (input: WordCountInput): number {
...
const calculateLineCount = function (input: WordCountInput): number {
  if (isWhitespaceString(input)) return 0
...
const calculateReadingTime = function (input: WordCountInput): number {
  if (isWhitespaceString(input)) return 0
  const wordCount: number = calculateWords(input)
...
const isWhitespaceString = (input: WordCountInput): boolean => !input.replace(/\s/g, '').length
